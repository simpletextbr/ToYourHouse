...
export default function Order() {
  ...
  async function closeShopping() {
    Animated.timing(carWidth, {
      toValue: 0,
      duration: 100,
    }).start();

    Animated.timing(carHeigth, {
      toValue: 0,
      duration: 400,
    }).start();
  }
  ...
  return (
    <View style={[styles.container, { backgroundColor: `${bgColor}` }]}>
      ...
      >
        ...
        >
          <TouchableOpacity style={styles.close} onPress={closeShopping}>
            <Feather
              name="x"
              size={20}
              color="#FFFFFF"
            ...
            renderItem={({ item: orders }) =>
              orders.length === 0 ? (
                ...
                <View style={styles.roworders}>
                  ...
                  <Text style={styles.priceShopping}>
                    {Intl.NumberFormat("pt-BR", {
                      style: "currency",
                      currency: "BRL",
                    }).format(orders.productvalue)}
                  </Text>
                </View>
              )
            }
      ...
      <View style={styles.rowbuttons}>
        <View style={styles.cardapio}>
          ...
          <TouchableOpacity
            style={[styles.cardapiobutton, { backgroundColor: `${btColor}` }]}
            onPress={() => goCardapio(enterprise)}
          >
        ...
        <TouchableOpacity
          style={[styles.shoppingCart, { backgroundColor: `${btColor}` }]}
          onPress={() => shopping()}
        >
          <Feather name="shopping-cart" size={36} color="#FFFFFF" />
        </TouchableOpacity>
        <TouchableOpacity
          style={[styles.nextbutton, { backgroundColor: `${btColor}` }]}
          onPress={() => next()}
        >
          <MaterialCommunityIcons
